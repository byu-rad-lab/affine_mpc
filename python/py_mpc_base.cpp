...
void moduleAddMPCBase(py::module& m)
{
  ...
  base.def("solve", &MPCBase::solve, "Solve optimization problem", py::arg("x0"));

  base.def("getNextInput",
  ...
  base.def("getParameterizedInputTrajectory",
  ...
  base.def("getParameterizedInputTrajectory",
    [](MPCBase& self)
    {
      Eigen::VectorXd u_traj_ctrl_pts{self.getNumInputs()*self.getNumControlPoints()};
      self.getParameterizedInputTrajectory(u_traj_ctrl_pts);
      return u_traj_ctrl_pts;
    }, "Get optimal parameterized trajectory of inputs from previous solve");

  base.def("getInputTrajectory",
  ...
  base.def("propagateModel",
    [](MPCBase& self, const Eigen::Ref<const Eigen::VectorXd>& x,
       const Eigen::Ref<const Eigen::VectorXd>& u, Eigen::Ref<Eigen::VectorXd> x_next)
    {
  ...
  base.def("setModelContinuous2Discrete", &MPCBase::setModelContinuous2Discrete,
    ...
    py::arg("Ac"), py::arg("Bc"), py::arg("wc"), py::arg("dt"), py::arg("tol")=1e-6);
  ...
  base.def("setReferenceParameterizedInputTrajectory", &MPCBase::setReferenceParameterizedInputTrajectory,
  ...
  base.def_property_readonly("len_horizon",
    [](MPCBase& self) { return self.getHorizonLength(); });
  base.def_property_readonly("num_ctrl_pts",
    [](MPCBase& self) { return self.getNumControlPoints(); });

// MPCBase can't work in Python anyways because derived classes resize Eigen variables
  // base.def("_convertToQP", static_cast<void (MPCBase::*)(const Eigen::Ref<const Eigen::VectorXd>&)>(&PyMPCBase::convertToQP),
