...
ImplicitMPC::ImplicitMPC(const int num_states, const int num_inputs,
    ...
    MPCBase(num_states, num_inputs, len_horizon, num_control_points,
                                 ...
                                 const Ref<const VectorXd>& x_max)
{
  ...
  A_.block(x_sat_idx_, 0, num_states_*len_horizon_, num_inputs_*num_ctrl_pts_) = S_;
...
void ImplicitMPC::calcSAndV(const Ref<const VectorXd>& x0)
{
  ...
  for (int k{1}; k < len_horizon_; ++k)
  {
    ...
    // S_.block(num_states_*k,0,num_states_,num_inputs_*num_ctrl_pts_) = Ad_ * S_.block(num_states_*(k-1),0,num_states_,num_inputs_*num_ctrl_pts_);
    S_.block(num_states_*k,0,num_states_,S_.cols()) = Ad_ * S_.block(num_states_*(k-1),0,num_states_,S_.cols());
    // S_.block(n*k,0,n,m*num_ctrl_pts_) = Ad_ * S_.block(n*(k-1),0,n,m*num_ctrl_pts_);
    S_.block(n*k,m*i,n,m) += (1-c) * Bd_;

    if (j < num_ctrl_pts_)
      S_.block(num_states_*k,num_inputs_*j,num_states_,num_inputs_) += c * Bd_;

    v_.segment(num_states_*k,num_states_) = Ad_*v_.segment(num_states_*(k-1),num_states_) + wd_;
  }
}
